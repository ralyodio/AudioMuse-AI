...
"""Container-aware CPU count for the worker thread caps.
...
blind to a ``--cpus`` limit. Every caller keeps its own formula and its own
floor; this module only decides which number goes into that formula, and any
doubt at all resolves to the caller's existing fallback.

Imports nothing outside the standard library on purpose. The RQ worker
entrypoints call it before importing any numeric library, because a BLAS thread
cap set after numpy loads has no effect at all, and pulling in ``joblib`` here
would load numpy and silently defeat the very cap it was meant to size.

Main Features:
* Reads cgroup v2 ``cpu.max`` and the cgroup v1 ``cfs_quota_us``/``cfs_period_us``
  pair, mirroring ``joblib.externals.loky.backend.context`` without importing it.
...
* ``usable_cpu_count`` feeds the ONNX ``intra_op_num_threads``, the only channel
  onnxruntime reads, since it ignores the BLAS environment variables, the affinity
  mask and the cgroup quota alike. It answers only when a real restriction was
  found below the host count; with no restriction it returns None and the caller
  leaves onnxruntime's own default untouched, so an unconstrained host keeps
  exactly the thread count it has today.
"""
...
def _probe_cpu_count():
    candidates = []
    for probe in (_cgroup_cpu_quota, _affinity_cpu_count, _host_cpu_count):
        try:
            value = probe()
        except Exception:
            value = None
...
def detect_cpu_count(fallback, minimum, label=''):
    ...
    try:
        ...
        logger.info(
            "%sCPU probe failed; keeping the usual count of %s", prefix, fallback
        )
